...
function makeTextureArrayProxy(textures) {
  return new Proxy(textures, {
    get(target, prop, receiver) {
      ...
      if (target[0] && prop in target[0]) {
        const val = target[0][prop];
        if (typeof val === 'function') return val.bind(target[0]);
        return val;
      }
      return undefined;
    }
...
class WebGLMultipleRenderTargets extends WebGLRenderTarget {
  constructor(width = 1, height = 1, count = 1, options = {}) {
    ...
    this._textureProxy = makeTextureArrayProxy(this.textures);
  }
  get texture() {
    return this._textureProxy;
  }
  set texture(val) {
...
class TemporalReprojectPass extends Pass {
  constructor(scene, camera, velocityDepthNormalPass, texture, textureCount, options = defaultTemporalReprojectPassOptions) {
    ...
    this.renderTarget = new WebGLMultipleRenderTargets(1, 1, textureCount, {
      ...
      type: (texture && typeof texture.type === 'number') ? texture.type : FloatType,
      depthBuffer: false
    });
...
const workerOnMessage = ({
  ...
  if (!data || !data.length) {
    postMessage({
      totalSumValue: 0,
      marginalDataArray: new Float32Array(height),
      conditionalDataArray: new Float32Array(width * height)
    });
    return;
  }
...
class EquirectHdrInfoUniform {
  ...
  updateFrom(map) {
    if (!map || !map.image) return Promise.resolve();
    map = map.clone();
    const width = (map.image && map.image.width) ? map.image.width : 0;
    const height = (map.image && map.image.height) ? map.image.height : 0;
    ...
    if (!data && typeof document !== 'undefined') {
      try {
        ...
        // Fallback for CORS/non-canvas textures
      }
    }
...
function isGizmoOrHelper(obj) {
  let cur = obj;
  while (cur) {
    if (cur.isTransformControls || cur.isTransformControlsGizmo || cur.isTransformControlsPlane || cur.isHelper || (cur.userData && cur.userData.isHelper) || cur.type === 'TransformControls' || cur.type === 'TransformControlsGizmo' || cur.type === 'TransformControlsPlane' || cur.type === 'GridHelper' || cur.name === 'GridHelper' || cur.name === 'ArrowHelper') {
      return true;
    }
    cur = cur.parent;
  }
...
class VelocityDepthNormalPass extends Pass {
  ...
  setVelocityDepthNormalMaterialInScene() {
    const rawVisible = getVisibleChildren$1(this._scene);
    this.visibleMeshes = [];

    for (const c of rawVisible) {
      if (isGizmoOrHelper(c)) {
        continue;
      }
      this.visibleMeshes.push(c);
      const originalMaterial = c.material;
  ...
  setSize(width, height) {
    ...
    this.lastVelocityTexture = new FramebufferTexture(w, h);
  ...
  render(renderer) {
    ...
    const targetWidth = Math.max(1, (this.renderTarget && this.renderTarget.width) || (renderer.domElement && renderer.domElement.width) || 512);
    const targetHeight = Math.max(1, (this.renderTarget && this.renderTarget.height) || (renderer.domElement && renderer.domElement.height) || 512);

    if (!this.lastVelocityTexture || !this.lastVelocityTexture.image || typeof this.lastVelocityTexture.image.width !== 'number' || this.lastVelocityTexture.image.width !== targetWidth || this.lastVelocityTexture.image.height !== targetHeight) {
      this.lastVelocityTexture = new FramebufferTexture(targetWidth, targetHeight);
      this.lastVelocityTexture.type = FloatType;
      this.lastVelocityTexture.minFilter = NearestFilter;
      this.lastVelocityTexture.magFilter = NearestFilter;
    }
...
class MotionBlurEffect extends Effect {
  ...
  update(renderer, inputBuffer, deltaTime) {
    ...
    const imgData = noiseTexture && (noiseTexture.image || (noiseTexture.source && noiseTexture.source.data));
